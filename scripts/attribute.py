...
for i, attribute in enumerate(attributes):
    ...
    if attribute['product_tmpl_id']:
        ...
        'product.template', 'search_read',[[('database_id_v9','=',attribute['product_tmpl_id'][0])]],{'offset': 0, 'limit': 1})
        if product_tmpl_id:
    ...
    if attribute['value_ids']:
        ...
        print('VALUES', values)
        for v in values:
            value_id = models_v_14.execute_kw(db_v_14, uid_v_14, password_v_14,
            'product.attribute.value', 'search_read',[[('name','=',v['name'])]],{'offset': 0, 'limit': 1})
            if value_id:
    ...
    try:
        ...
        print("Processing [%s] %s of %s [%s] %s"%(attribute['id'], i, size, 100 * i/size, '%'))
