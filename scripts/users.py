...
for i, user in enumerate(users):
    ...
    if user['partner_id']:
        ...
        partners = models_v_14.execute_kw(db_v_14, uid_v_14, password_v_14,
        'res.partner', 'search_read',[[('database_id_v9', '=', partner_id)]])
        if partners:
            user['partner_id'] = partners[0]['id']
        else:
            del user['partner_id']
    del user['image_small']
    print("Processing [%s] %s of %s [%s] %s"%(user['id'], i, size, 100 * i/size, '%'))
    id = models_v_14.execute_kw(db_v_14, uid_v_14, password_v_14, 'res.users', 'create', [user])
    
    print("Processed user id [%s] %s of %s [%s] %s"%(id, i, size, 100 * i/size, '%'))
