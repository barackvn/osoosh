...
class CrafitoEcommerceShop(WebsiteSale):
    ...
    def cart_update_json(self, product_id, line_id=None, add_qty=None, set_qty=None, display=True):
        ...
        result.update({'theme_crafito.hover_total': request.env['ir.ui.view']._render_template("website_sale.cart_popover", {
    ...
    def _get_crafito_products_recently_viewed(self):
        ...
        if visitor := request.env['website.visitor']._get_visitor_from_request():
            ...
            if products_ids := [product['product_id'][0] for product in products]:
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        if request.env['website'].sudo().get_current_website().theme_id.name == 'theme_crafito':
            ...
            if session.get('sort_id'):
                ...
                sort_order = 'is_published desc, %s %s, id desc' % (order_field, order_type)
                if post.get("sort_id"):
